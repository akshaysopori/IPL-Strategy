...
-- Question 6
SELECT player_name, ROUND(runs_scored/9,1) AS avg_runs_per_season FROM (
...
-- Question 7
SELECT player_name, ROUND(wickets_taken/9,0) AS avg_wickets_per_season FROM (
...
FROM runs_scored_by_rcb a
JOIN wickets_taken_by_rcb b ON a.season_id = b.season_id ;
...
DENSE_RANK() OVER( ORDER BY ROUND(AVG(wickets),1) DESC ) AS bowler_ranking
...
-- Question 15
SELECT  m.venue_id, ROUND(SUM(runs_scored)/COUNT( distinct m.match_id),2) AS average, player_name
FROM ball_by_ball b 
JOIN batsman_scored bt ON b.match_id = bt.match_id AND b.over_id = bt.over_id AND bt.ball_id = b.ball_id
AND bt.innings_no = b.innings_no
JOIN matches m ON b.match_id = m.match_id JOIN player p ON striker = p.player_id WHERE season_id IN (6,7,8,9)
AND player_name IN (
